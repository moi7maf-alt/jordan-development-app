...
# تحميل البيانات من الملفات المرفقة
@st.cache_data
def load_all_data():
    """تحميل جميع البيانات الرسمية من المصادر المرفقة"""
    ...
    education_data = {
        'المحافظة': ['عمان', 'المفرق', 'إربد', 'الزرقاء', 'البلقاء', 'مأدبا', 'معان', 'الكرك', 'جرش', 'عجلون', 'العقبة', 'الطفيلة'],
        'المدارس': [2430, 695, 1455, 860, 500, 252, 240, 418, 293, 234, 165, 151],
        'المستأجرة': [250, 190, 545, 270, 140, 70, 15, 100, 100, 75, 52, 33],
        'الفترتين': [1015, 35, 320, 175, 128, 10, 14, 14, 14, 4, 4, 2],
        'الطلبة': [850200, 165000, 425000, 312000, 156000, 63000, 48000, 91500, 72000, 52000, 56000, 33000],
        'المعلمين': [25600, 10500, 16400, 11100, 6600, 3160, 3860, 6880, 3730, 2890, 2520, 2530],
        'طالب_معلم': [33.21, 15.71, 25.91, 28.11, 23.64, 19.94, 12.44, 13.3, 19.3, 17.99, 22.22, 13.04],
    ...
    health_data = {
        'المحافظة': ['عمان', 'المفرق', 'إربد', 'الزرقاء', 'البلقاء', 'مأدبا', 'معان', 'الكرك', 'جرش', 'عجلون', 'العقبة', 'الطفيلة'],
        'مستشفيات_وزارة_الصحة': [3, 4, 8, 2, 5, 2, 2, 2, 1, 1, 1, 1],
        'أسرة_وزارة_الصحة': [2010, 306, 932, 663, 894, 161, 232, 267, 136, 250, 20, 188],
        'مستشفيات_حكومية_أخرى': [10, 1, 2, 1, 0, 0, 0, 1, 0, 1, 1, 1],
        'أسرة_حكومية_أخرى': [2453, 164, 1112, 200, 0, 0, 0, 183, 0, 183, 201, 122],
    ...
    water_data = {
        'المحافظة': ['عمان', 'المفرق', 'إربد', 'الزرقاء', 'البلقاء', 'مأدبا', 'معان', 'الكرك', 'جرش', 'عجلون', 'العقبة', 'الطفيلة'],
        'حصة_الفرد_2023': [125.4, 150.3, 77.6, 103.0, 242.5, 134.5, 222.2, 157.8, 84.3, 85.1, 302.9, 202.8],
    ...
    waste_data = {
        'المحافظة': ['عمان', 'المفرق', 'إربد', 'الزرقاء', 'البلقاء', 'مأدبا', 'معان', 'الكرك', 'جرش', 'عجلون', 'العقبة', 'الطفيلة'],
        'كمية_النفايات_2022': [1360527.5, 259918.7, 865147.3, 502484.6, 492095.1, 123308.1, 31218.9, 184386, 104342.8, 112801, 23671, 122260.4],
    ...
    for _, province in high_risk.iterrows():
        with st.expander(f"🚩 {province['المحافظة']} - مستوى الخطر: {province['مستوى_الخطر']}/10", expanded=True):
            ...
            if "عجز مائي" in str(province['الإنذارات']):
                ...
                st.write("• تطوير مشاريع收集 مياه الأمطار")
...
def strategic_planning_insights():
    ...
    for _, row in df.iterrows():
        ...
        priority_indicators.append({
            ...
            'العجز_المائي': row['العجز_المائي_٪']
        })
...
def sectoral_analysis():
    ...
    with col1:
        # الكثافة الصحية
        df['كثافة_الأسرة'] = (df['أسرة_وزارة_الصحة'] + df['أسرة_حكومية_أخرى']) / df['التعداد_2024'] * 10000
        fig3 = px.bar(
            df.sort_values('كثافة_الأسرة', ascending=False),
            x='المحافظة',
            y='كثافة_الأسرة',
            title='كثافة الأسرة الصحية (لكل 10,000 نسمة)',
            color='كثافة_الأسرة'
        )
...
def predictive_analytics():
    ...
    st.header("🔮 التحليلات التنبؤية واستشراف المستقبل")
    
    df = load_all_data()
    
    st.subheader("📈 توقعات المؤشرات التنموية 2025-2030")
    
    # محاكاة تنبؤية مبسطة
    simulation_data = []
    
    for _, row in df.iterrows():
        # افتراضات النمو/الانخفاض بناءً على الاتجاهات
        unemployment_trend = -0.5 if row['البطالة_2024'] > 22 else -0.3
        participation_trend = 0.8 if row['مشاركة_الإناث'] < 15 else 0.4
        
        for year in range(2025, 2031):
            # محاكاة التغير في البطالة
            future_unemployment = max(10, row['البطالة_2024'] + (unemployment_trend * (year - 2024)))
            
            # محاكاة التغير في المشاركة الاقتصادية للإناث
            future_female_participation = min(35, row['مشاركة_الإناث'] + (participation_trend * (year - 2024)))
            
            simulation_data.append({
                'المحافظة': row['المحافظة'],
                'السنة': year,
                'البطالة_المتوقعة': future_unemployment,
                'مشاركة_الإناث_المتوقعة': future_female_participation
            })
    
    sim_df = pd.DataFrame(simulation_data)
    
    # عرض التوقعات
    selected_province = st.selectbox("اختر المحافظة:", df['المحافظة'].unique())
    
    province_data = sim_df[sim_df['المحافظة'] == selected_province]
    
    fig = px.line(
        province_data,
        x='السنة',
        y=['البطالة_المتوقعة', 'مشاركة_الإناث_المتوقعة'],
        title=f'التوقعات التنموية لمحافظة {selected_province} (2025-2030)',
        markers=True
    )
    st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    if menu_option == "اللوحة التنفيذية":
        ...
        sectoral_analysis()
    elif menu_option == "التحليلات التنبؤية":
        predictive_analytics()
    elif menu_option == "تقارير القيادة":
        st.header("📋 تقارير القيادة التفصيلية")
        st.info("""
        ### تقارير القيادة المتاحة:
        - تقرير الأداء التنموي الشهري
        - تقرير مؤشرات الرؤية 2033
        - تقرير التنفيذ والمتابعة
